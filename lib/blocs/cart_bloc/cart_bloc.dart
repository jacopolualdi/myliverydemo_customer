...
class CartBloc extends Bloc<CartEvent, CartState> {
  ...
  Stream<CartState> mapEventToState(CartEvent event) async* {
    if (event is AddToCartEvent) {
      yield* mapAddToCartEventToState(
        event.productId,
        event.uid,
        event.addedExtras,
        event.seletedSize,
    ...
    if (event is IncreaseQuantityEvent) {
      yield* mapIncreaseQuantityEventToState(
          event.quantity, event.uid, event.productId,event.itemId);
    }
  ...
  Stream<CartState> mapAddToCartEventToState(
    ...
    try {
      //add to cart
      bool isAdded = await userDataRepository.addToCart(
        productId,
        uid,
        addedExtras,
        seletedSize,
  ...
  Stream<CartState> mapIncreaseQuantityEventToState(
    ...
    try {
      bool increased =
          await userDataRepository.increaseQuantity(quantity, uid, productId,itemId);
      if (increased) {
        // add(GetCartProductsEvent(uid));
