...
class _ProductDetailItemState extends State<ProductDetailItem> {
  ...
  void initState() {
    ...
    totalCost = double.parse(widget.product.price);
    ...
    rateProductBloc.listen((state) {
        ...
        if (checkRatingProduct) {
          ...
          if (state.result != null) {
            ...
            if (state.result == 'NOT_ORDERED') {
              //not ordered
              showSnack('Non puoi recensire questo prodotto, devi prima ordinarlo almeno una volta.', context);
            }
          } else {
      ...
      if (state is RateProductInProgressState) {
        ...
        Navigator.pop(context);
        showUpdatingDialog('Pubblico recensione...\nAttendere...');
      }
      if (state is RateProductFailedState) {
        //show failed popup
        if (isRatingProduct) {
          Navigator.pop(context);
          showSnack('Recensione non riuscita.', context);
          isRatingProduct = false;
        }
      }
      ...
      children: <Widget>[
        ...
        Padding(
          ...
          child: Row(
            ...
            children: <Widget>[
              Text(
                widget.product.inStock ? 'Disponibile' : 'Esaurito',
                maxLines: 1,
                style: GoogleFonts.poppins(
                  fontSize: 15.0,
                ...
                children: [
                  ...
                  ListView.separated(
                    ...
                    itemBuilder: (context, index) => RadioListTile(
                      value: index,
                      groupValue: seletedSize != null ? seletedSize : -1,
                      onChanged: (val) {
                        setState(() {
                          seletedSize = val;

                          totalCost = double.parse(
                              widget.product.sizes[seletedSize].price);
                      ...
                      dense: true,
                      title: Text(
                        '${widget.product.sizes[index].name} - ${Config().currency}${widget.product.sizes[index].price}',
                        style: GoogleFonts.poppins(
                          fontSize: 13.5,
                ...
                children: [
                  ...
                  ListView.separated(
                    ...
                    itemBuilder: (context, index) => CheckboxListTile(
                      value: addedExtras.contains(widget.product.extras[index]),
                      selected:
                          addedExtras.contains(widget.product.extras[index]),
                      activeColor: Colors.black87,
                      onChanged: (val) {
                        if (val) {
                          setState(() {
                            addedExtras.add(widget.product.extras[index]);

                            totalCost = totalCost +
                                double.parse(
                                    widget.product.extras[index].price);
                          });
                        } else {
                          setState(() {
                            addedExtras.remove(widget.product.extras[index]);
                      ...
                      dense: true,
                      title: Text(
                        '${widget.product.extras[index].name} (+ ${widget.product.extras[index].price} ${Config().currency})',
                        style: GoogleFonts.poppins(
                          fontSize: 12.0,
                          fontWeight: FontWeight.w400,
                          letterSpacing: 0.5,
