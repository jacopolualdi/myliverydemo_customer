...
class _CartScreenState extends State<CartScreen> {
    ...
    manageCouponsBloc.listen((state) {
      ...
      if (state is ApplyCouponFailedState) {
        ...
        if (isApplying) {
          isApplying = false;
          Navigator.of(context).pop();
          showSnack('Applicazione Cod. Sconto non riuscita.', context);
        }
      }
      ...
      if (state is ApplyCouponCompletedState) {
        if (isApplying) {
          ...
          if (state.res.couponCode != null) {
            ...
            //not found
            showSnack('Codice Sconto non valido', context);
          }
        }
      }
    });
    ...
    return showDialog(
      ...
      builder: (context) {
        return ProcessingDialog(
          message: 'Applico Codice Sconto...\nAttendere...',
        );
      },
    );
  }

  void checkClosingHours() {
    //check closing hours
    ClosingHour todaysClosingHour;
    String todaysDay;
    DateTime now = DateTime.now();

    todaysDay = DateFormat('E').format(now);

    print(todaysDay);
    print(now.hour);
    print(now.minute);

    for (var item in cartValues.closingHours.closingHours) {
      if (item.day.toLowerCase() == todaysDay.toLowerCase()) {
        todaysClosingHour = item;
        break;
      }
    }

    //testing
    DateTime openLunch = DateFormat('hh:mm').parse(
        '${todaysClosingHour.lunch.from.toDate().hour}:${todaysClosingHour.lunch.from.toDate().minute}');
    openLunch = new DateTime(
        now.year, now.month, now.day, openLunch.hour, openLunch.minute);
    DateTime closeLunch = DateFormat('hh:mm').parse(
        '${todaysClosingHour.lunch.to.toDate().hour}:${todaysClosingHour.lunch.to.toDate().minute}');
    closeLunch = new DateTime(
        now.year, now.month, now.day, closeLunch.hour, closeLunch.minute);

    DateTime openDinner = DateFormat('hh:mm').parse(
        '${todaysClosingHour.dinner.from.toDate().hour}:${todaysClosingHour.dinner.from.toDate().minute}');
    openDinner = new DateTime(
        now.year, now.month, now.day, openDinner.hour, openDinner.minute);
    DateTime closeDinner = DateFormat('hh:mm').parse(
        '${todaysClosingHour.dinner.to.toDate().hour}:${todaysClosingHour.dinner.to.toDate().minute}');
    closeDinner = new DateTime(
        now.year, now.month, now.day, closeDinner.hour, closeDinner.minute);

    if ((now.isAfter(openLunch) && now.isBefore(closeLunch)) ||
        (now.isAfter(openDinner) && now.isBefore(closeDinner))) {
      print('YES');
      Navigator.push(
        context,
        MaterialPageRoute(
      ...
      print('NO');
      showSnack('Spiacenti, siamo chiusi in questo momento.', context);
    }
  }
  ...
  applyCoupon() {
    ...
    for (var product in cartProducts) {
      totalOrderAmt = totalOrderAmt +
          (double.parse(product.totalPrice) * int.parse(product.quantity));
    }
    ...
    setState(() {
      if (coupon.type == 'LIMITED_USE_COUPON') {
        ...
        if (coupon.usedNoOfTimes < int.parse(coupon.noOfUses)) {
          ...
          //dont allow
          showSnack('Codice Sconto scaduto', context);
        }
      } else {
        //limited time
        DateTime now = DateTime.now();
                            ...
                            if (state is GetCartProductsCompletedState) {
                              ...
                              return Container(
                                ...
                                padding: const EdgeInsets.symmetric(
                                    vertical: 5.0, horizontal: 12.0),
                                child: Text(
                                  'Prodotti: ${state.cartProductsList.length}',
                                  style: GoogleFonts.poppins(
                                    color: Colors.white,
                                    fontSize: 15.0,
                                    fontWeight: FontWeight.w500,
                                    ...
                                    if (appliedCoupon) {
                                      ...
                                      if (coupon.type == 'LIMITED_USE_COUPON') {
                                        ...
                                        if (now.isAfter(
                                          ...
                                          print('NO');
                                          showSnack(
                                              'Codice Sconto non valido', context);
                                        }
                                      }
                                    } else {
                                      totalAmt = 0;
                                            ...
                                            children: <Widget>[
                                              Row(
                                                ...
                                                children: <Widget>[
                                                  ...
                                                  Expanded(
                                                    child: Text(
                                                      'Ottieni il ${cartValues.cartInfo.discountPer}% di sconto per ordini superiori a ${Config().currency}${cartValues.cartInfo.discountAmt}',
                                                      style:
                                                          GoogleFonts.poppins(
                                                        color: Colors.black87,
                                                        fontSize: 15.5,
                        ...
                        onPressed: () {
                          ...
                          for (var item in cartProducts) {
                            if (!item.product.inStock) {
                              ...
                              showSnack(
                                  'Nel tuo carrello ci sono prodotti esauriti.\nPer favore, rimuovili.',
                                  context);
                              return;
                            }
                          }
                          ...
                          if (totalAmt <
                              double.parse(cartValues.cartInfo.minAmt)) {
                            showSnack(
                                'È richiesto un minimo d\'ordine di ${Config().currency}${cartValues.cartInfo.minAmt}',
                                context);
                            return;
                          }
                          checkClosingHours();
