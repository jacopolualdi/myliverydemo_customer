...
exports.newOrderNotif = functions.firestore.document('Orders/{orderId}').onCreate(async (doc, context) => {
    ...
    payload = {
        notification: {
            ...
            body: 'Abbiamo ricevuto il tuo ordine!',
            sound: 'default',
        },
        data: {
            type: 'orderStatus',
...
exports.notifyCustomer = functions.firestore.document('Orders/{orderId}').onUpdate(async (change, context) => {
    ...
    if (change.before.data().orderStatus === 'In Preparazione' && change.after.data().orderStatus === 'In Consegna') {
        // payload = {
        //     data: {
        //         orderId: orderId,
        //         orderStatus: orderStatus,
        ...
        payload = {
            notification: {
                title: 'Ordine #' + orderId,
                body: 'Il tuo ordine è ' + orderStatus,
                sound: 'default',
            },
            data: {
                type: 'orderStatus',
        ...
        (change.before.data().orderStatus === 'In Consegna' || change.before.data().orderStatus === 'In Preparazione') &&
        change.after.data().orderStatus === 'Consegnato'
    ) {
        payload = {
            notification: {
                title: 'Ordine #' + orderId,
        ...
        (change.before.data().orderStatus === 'In Consegna' ||
            change.before.data().orderStatus === 'In Preparazione' ||
            change.before.data().orderStatus === 'Consegnato') &&
        change.after.data().orderStatus === 'Cancellato'
    ) {
...
exports.updateOrderAnalytics = functions.firestore.document('Orders/{orderId}').onUpdate(async (change, context) => {
    ...
    if (change.before.data().orderStatus === 'In Preparazione' &&
        change.after.data().orderStatus === 'Preparato' || change.after.data().orderStatus === 'In Consegna'
    ) {
    ...
    } else if (change.before.data().orderStatus === 'In Preparazione' &&
        change.after.data().orderStatus === 'Cancellato') {
    ...
    } else if ((change.before.data().orderStatus === 'Preparato' || change.before.data().orderStatus === 'In Consegna') &&
        change.after.data().orderStatus === 'Cancellato') {
    ...
    } else if (change.before.data().orderStatus === 'In Consegna' &&
        change.after.data().orderStatus === 'Consegnato'
    ) {
...
exports.sellerOrderUpdateNotif = functions.firestore.document('Orders/{orderId}').onUpdate(async (change, context) => {
    ...
    if (change.before.data().orderStatus === 'In Preparazione' && change.after.data().orderStatus === 'In Consegna') {
        ...
        payload = {
            notification: {
                title: 'Ordine #' + orderId,
                body: 'Questo ordine è ' + orderStatus,
                sound: 'default',
            },
            data: {
                type: 'orderStatus',
...
exports.userReportNotif = functions.firestore.document('UserReports/{reportId}').onCreate(async (snap, context) => {
    ...
    payload = {
        notification: {
            title: 'Prodotto Segnalato',
            body: `Il prodotto #${productId} è stato segnalato: "${report}"`,
            sound: 'default',
        },
        data: {
            type: 'userReport',
...
exports.delveryUserOrderAssignedNotif = functions.firestore.document('Orders/{orderId}').onUpdate(async (change, context) => {
    ...
    if (change.before.data().orderStatus === 'In Preparazione' || change.before.data().orderStatus === 'Preparato' &&
        change.after.data().orderStatus === 'In Consegna') {

        payload = {
            notification: {
                title: 'Ordine #' + orderId,
                body: 'NUOVO ORDINE ASSEGNATO',
                sound: 'default',
            },
            data: {
                type: 'orderAssigned',
